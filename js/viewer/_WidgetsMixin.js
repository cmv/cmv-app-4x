define([
    ...
    return declare(null, {
        ...
        widgets: {},
        widgetTypes: ['contentPane', 'domNode', 'expandPane', 'floating', 'invisible', 'layer', 'layout', 'loading', 'view', 'titlePane', 'ui'],
        ...
        startup: function () {
            ...
            if (this.mapViewDeferred) {
                this.mapViewDeferred.then(lang.hitch(this, 'createWidgets', ['view', 'layer']));
            }
            if (this.layoutDeferred) {
                this.layoutDeferred.then(lang.hitch(this, 'createWidgets', ['layout']));
                promiseAll([this.mapViewDeferred, this.layoutDeferred])
        ...
        _setWidgetOptions: function (widgetConfig, options) {
            ...
            if (options.view) {
                options.view = this.mapView;
            }

            //Show errors and more from widgets by the growler widget
            if (options.growler) {
                options.growler = this.widgets['growler'];
            }

            if (options.mapViewRightClickMenu) {
                // create right-click menu
                if (!this.mapViewRightClickMenu) {
                    this.mapViewRightClickMenu = new Menu({
                        targetNodeIds: [this.mapView.root]//,
                        //selector: '.esriMapLayers' // restrict to map only
                        //selector: '.esri-overlay-surface' // restrict to map only
                    });
                    this.mapViewRightClickMenu.startup();
                }
                options.mapViewRightClickMenu = this.mapViewRightClickMenu;
            }
            if (options.mapViewClickMode) {
                options.mapViewClickMode = this.mapViewClickMode.current;
            }
        ...
        _placeWidget: function (widget, pnl, widgetConfig) {
            ...
            if (placeAt) {
                ...
                if (widgetConfig.expendable)
                {
                    ...
                    if (widgetConfig.collapse) {
                        bgExpand.collapse();
                    }

                    this.mapView.ui.add(bgExpand, {
                        position: widgetConfig.placeAt,
                        index: widgetConfig.position
                    });
                }
                else
                {
