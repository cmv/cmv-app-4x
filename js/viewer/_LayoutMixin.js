define([
    ...
    return declare(null, {
        ...
        startup: function () {
            ...
            this.mapViewDeferred.then(lang.hitch(this, 'createPanes'));
        ...
        addTopics: function () {
            ...
            // set the current mapViewClickMode
            topic.subscribe('mapViewClickMode/setCurrent', lang.hitch(this, function (mode) {
                this.mapViewClickMode.current = mode;
                topic.publish('mapViewClickMode/currentSet', mode);
            }));

            // set the current mapClickMode to the default mode
            topic.subscribe('mapViewClickMode/setDefault', lang.hitch(this, function () {
                topic.publish('mapViewClickMode/setCurrent', this.mapViewClickMode.defaultMode);
            }));
            ...
            for (key in panes) {
                if (panes.hasOwnProperty(key)) {
                    ...
                    if (key !== 'center' && this.panes[key]._splitterWidget) {
                        domClass.add(this.mapView.root.parentNode, 'pane' + key);
        ...
        setPhoneInfoWindow: function () {
            /*var hasphone = (navigator.platform == 'Win32' || navigator.platform == 'Win64') ? false : true;

           // has.add('phone', hasphone);

            if (has('phone') && !this.config.mapOptions.infoWindow) {
                this.config.mapOptions.infoWindow = new Popup(null, put('div'));
            }*/
        }
    });
});
