...
define([
    ...
    var PrintDijit = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
        ...
        postCreate: function () {
            ...
            this.printparams.view = this.view;
            this.printparams.outSpatialReference = (this.outWkid && typeof this.outWkid !== 'undefined') ? new SpatialReference(this.outWkid) : this.view.spatialReference;

           // this.printTask = new print(this.printTaskURL, this.printparams);            

            if (this.showLayout) {
                this.mapUnitsToMeters = this.getUnitToMetersFactor(this.view.spatialReference.unit); //Print Plus Enhancements (this may break in future API releases)
                if (isNaN(this.mapUnitsToMeters.x) || isNaN(this.mapUnitsToMeters.y)) {
                    this.showLayout = false;
                } else if (this.view.spatialReference.unit === 'degrees' || this.view.spatialReference.isWebMercator) {
                    this.printGL = new GraphicsLayer({
                        id: 'printGraphics',
                        opacity: 1.0
                    });

                    this.view.map.add(this.printGL);
                 //   this.printGL.enableMouseEvents(); //Print Plus Enhancements                    

                   // sketchViewModel.on(["update", "undo", "redo"], onGraphicUpdate);

                    var e = this.view.extent; // change this to get the initial extent???
                    var lineN = new Polyline({ spatialReference: this.view.spatialReference });
                    lineN.addPath([[e.xmin, e.ymax], [e.xmax, e.ymax]]);
                    var lineS = new Polyline({ spatialReference: this.view.spatialReference });
                    lineS.addPath([[e.xmin, e.ymin], [e.xmax, e.ymin]]);
                    var lineE = new Polyline({ spatialReference: this.view.spatialReference });
                    lineE.addPath([[e.xmax, e.ymax], [e.xmax, e.ymin]]);
                    var lineW = new Polyline({ spatialReference: this.view.spatialReference });
                    ...
                    //lp.lengthUnit = geomService.UNIT_METER;
                    lp.lengthUnit = 'meters'; 

                    lp.geodesic = true;

                    var handle = geometryService.lengths(esriConfig.geometryServiceUrl, lp);
                    handle.then(
                    ...
                    geometryService.lengths(esriConfig.geometryServiceUrl, lp,
                        lang.hitch(this, function (result) {
                            if (result.lengths.length === 4) {
                                var southRatio = (result.lengths[0] / eDims.x);
                                var northRatio = (result.lengths[1] / eDims.x);
                ...
                setUpGraphicClickHandler(this.view, this.sketchViewModel);
            ...
            */
            ...
            var url = this.printTaskURL + '?f=pjson';
            var options = {
                responseType: 'json',
                method: 'post',
                useProxy: false,
            ...
            var handle = esriRequest(url, options);
            handle.then(
	    ...
	    //    esriRequest.setRequestPreCallback(myCallbackFunction);
        	...
        	urls: this.printTaskURL,
        	// use the BeforeInterceptorCallback to add token to query
        	before: function(params) {
          	    //params = this.myCallbackFunction(params)

          	    if (params.url.indexOf("/submitJob") > -1) {
               		var strjson = JSON.parse(params.requestOptions.query.Web_Map_as_JSON);

               		array.forEach(strjson.operationalLayers, function (ol) {
               		// if (ol.id != "basemap") {
               		if (ol.url != null) { 
                           var newurl = ol.url;
                           // TODO reverse
               		...
               		var jsontxt = JSON.stringify(strjson );
              		params.requestOptions.query.Web_Map_as_JSON = jsontxt;
          	    }
          	    //if (args.url.indexOf("/jobs/") > -1) {
               		//var newurl = setLocalIP_Str(args.url);
               		//args.url = newurl; 
          	    //}
        	},
            });
        ...
        function setUpGraphicClickHandler(view, sketchViewModel) {
            view.on("click", (event) => {
            ...
            /*
            view.on("pointer-up", (event, mapAreaCenter) => {
                if (sketchViewModel.state === "active") {
                    view.hitTest(event, mapAreaCenter).then((response) => {
                        //const results = response.results;
                        // Check if the new development graphic was clicked and pass
                        // the graphic to sketchViewModel.update() with reshape tool.
                        //results.forEach((result) => {
                        mapAreaCenter = sketchViewModel.updateGraphics._items[0].geometry.extent.center;
                        //});
                    });
                }
            });*/
	    ...
	    function myCallbackFunction(args) {
            if (args.url.indexOf("/submitJob") > -1) {
               var jsonObj = JSON.parse(args.content.Web_Map_as_JSON);
               var jsontxt = JSON.stringify(setLocalIP_Json(jsonObj));
        ...
        function setLocalIP_Json(strjson) {
              array.forEach(strjson.operationalLayers, function (ol) {
               // if (ol.id != "basemap") {
              if (ol.url != null) { 
                   var newurl = ol.url;
        ...
        initializeFeatureLayers: function () {
            ...
            array.forEach(this.view.map.layers.items, lang.hitch(this, function (layer) {
                //var l = this.map.getLayer(item);
                if (layer.hasOwnProperty('type') && layer.type === 'feature') {
        ...
        _handlePrintInfo: function (data) {
            ...
            */
            ...
            var layoutTemplate = array.filter(data.data.parameters, function (param) {
            ...
            var Format = array.filter(data.data.parameters, function (param) {
        ...
        getmapAreaCenter: function () {
            if (this.printGL && this.printGL.graphics.items && this.printGL.graphics.items.length > 0) {
                this.mapAreaCenter = this.printGL.graphics.items[0].geometry.extent.center;
            } else if (!this.mapAreaCenter) {
                //
            } else {
                this.mapAreaCenter =  this.view.extent.center;
            }
            return this.mapAreaCenter;
        },
        ...
        print: function () {
            if (!this.printGL) {
                ...
                this.oldExtent = this.view.extent;
                this.oldOpacity = this.printGL.opacity;
                this.printGL.opacity = 0;

              //  this.printGL.refresh();

                this.oldLODs = null;
                this.suspendExtentHandler = true; //Suspend the Extent change handler
                var deferred = null;

                var zoom = printScale / this.view.scale;
                var extent = this.view.extent;
                extent.expand(zoom);
                extent.centerAt(this.getmapAreaCenter());

                deferred = this.view.goTo(extent);
                ...
                //}
                /*
                if (this.view.map.basemap.baseLayers.items && this.view.map.basemap.baseLayers.items.length >= 0) {
                    var tileInfo = this.view.map.basemap.baseLayers.items[0].tileInfo;
                    if (tileInfo && tileInfo.lods)
                    {
                        this.oldLODs = this.view.map.basemap.baseLayers.items[0].tileInfo.lods;
                        // Set one LOD for the printing scale and zoom to it.
                        var resolution = printScale / (this.oldLODs[0].scale / this.oldLODs[0].resolution);
                        this.view.map.basemap.baseLayers.items[0].tileInfo.lods = [{
                            'level': 0,
                            'resolution': resolution,
                            'scale': printScale
                        }];
                        ...
                        this.view.zoom = 0;
                        deferred = this.view.goTo(this.mapAreaCenter);
                    } else {
                        // Zoom to the print scale
                        this.view.zoom = printScale / this.view.scale;
                        deferred = this.view.goTo(this.mapAreaCenter);
                    }                    
                } else {
                    // Zoom to the print scale
        ...
        printAndRestoreSettings: function () {
            ...
            if (this.printRequested && layersSet) {
                ...
                if (this.oldLODs) {
                    this.view.map.basemap.baseLayers.items[0].tileInfo.lods = this.oldLODs;
                }
                var deferred = this.view.goTo(this.oldExtent);
        ...
        submitPrintJob: function () {
            if (this.printSettingsFormDijit.isValid()) {
                ...
                if (mapOnlyForm.printUnits === 'esriCentimeters') {
                ...
                var fileHandel = print.execute(this.printTaskURL, this.printparams);
        ...
        _onTitleBlockChange: function () {
            ...
            this.printGL.removeAll();
            this.setScaleRanges();
            //this.drawMapSheet(this.mapAreaCenter);
            this.drawMapSheet(this.getmapAreaCenter());
        },
        ...
        _onLayoutChange: function (layout) {
            ...
            if (layout === 'MAP_ONLY') {
                ...
                if (!this._zoomEndHandler) {
                    //this._zoomEndHandler = this.view.on('zoom-end', lang.hitch(this, 'adjustLayoutToMap'));
                    this._zoomEndHandler = this.view.on('mouse-wheel', lang.hitch(this, 'adjustLayoutToMap'));
                }
                if (!this._panEndHandler) {
                    //this._panEndHandler = this.view.on('pan-end', lang.hitch(this, 'adjustLayoutToMap'));
                    //this._panEndHandler = this.view.on('hold', lang.hitch(this, 'adjustLayoutToMap'));
                    this._panEndHandler = this.view.on('drag', lang.hitch(this, 'getmapAreaCenter'));                    
                }
                ...
                var centerPt = this.view.extent.center;
                //var printScale = this.scaleSliderDijit.get('value');
                this.setScaleRanges();
                ...
                this.printGL.on('mouse-drag', lang.hitch(this, function (evt) {
                    if (evt.graphic.id === this.mapSheetGraphicId && this.mapSheetMoving) {
                        ...
                        if (!this.view.extent.contains(this.mapAreaExtent)) {
                ...
                this.printGL.on('mouse-over', lang.hitch(this, function (evt) {
                    if (evt.graphic.id === this.mapSheetGraphicId) {
                        this.view.setMapCursor('move');
                    }
                }));
                this.printGL.on('mouse-out', lang.hitch(this, function (evt) {
                    if (evt.graphic.id === this.mapSheetGraphicId) {
                        this.view.setMapCursor('default');
                    }
                    this.view.enablePan();
        ...
        drawMapSheet: function (centerPt) {
            ...
            this.mapAreaExtent = new Extent(minX, minY, maxX, maxY, this.view.spatialReference);
        ...
        _adjustMapSketch: function () {
            ...
            var browserAspectRatio = this.view.width / this.view.height;
            ...
            if (preserveScale) {
                ...
                browserMapDims = new Dims(this.view.extent.width / scale / unitRatio.x, this.view.extent.height / scale / unitRatio.y);
        ...
        adjustLayoutToMap: function (evt) {
            if (!this.suspendExtentHandler) {
                ...
                //if (evt.hasOwnProperty('level')) {
                    ...
                    if (!this.mapAreaCenter) {
                        centerPt = this.view.center;
                    } else {
                        centerPt = this.mapAreaCenter;
                    }

                    // This was a zoom, so reset the scales and redraw the map sheet
                    this.setScaleRanges();
                    //centerPt = evt.extent.center;
                    this.printGL.removeAll();
                    this.drawMapSheet(centerPt);
                //}
        ...
        adjustMapToLayout: function () {
            ...
            this.view.extent = this.view.extent.offset(-correction.x, -correction.y);
            this.suspendExtentHandler = false;
        },
        ...
        getCorrection: function () {
            ...
            var mapExtent = this.view.extent;
        ...
        setScaleRanges: function () {
            ...
            if (!isNaN(unitRatio.x) && !isNaN(unitRatio.y)) {
                if (titleBlock) {
                    maxScale = Math.ceil(Math.min(mapExtent.height / mapSize.y / unitRatio.y,
                        mapExtent.width / mapSize.x / unitRatio.x));
                } else {
                    maxScale = Math.ceil(Math.min(mapExtent.height / (pageSize.y - titleBlockOffsets.y * 2) / unitRatio.y,
                        mapExtent.width / (pageSize.x - titleBlockOffsets.x * 2) / unitRatio.x));
                }
            } else {
            ...
            function getLabels (minScale, snapInterval, scaleIndices) {
                ...
                if (scaleIndices && scaleIndices.length > 0) {
