    ...
    var _DynamicFolder = declare([WidgetBase, TemplatedMixin], {
        ...
        postCreate: function () {
            ...
            if (!this.control.controlOptions.ignoreDynamicGroupVisibility) {
                ...
                var tlayerViz = [];
                if (this.control.layer.sublayers && this.control.layer.sublayers.length > 0) {
                    array.forEach(this.control.layer.sublayers.items, function (item) {
                        if (item.visible) {
        ...
        _getSubLayerNodes: function () {
            ...
            var subLayerNodes = null;
            try {
                subLayerNodes = query('.' + this.control.layer.id + '-layerControlSublayerCheck', this.domNode);
                ...
                return array.filter(subLayerNodes, lang.hitch(this, function (node) {
                    ...
                    var subLayerIds = array.map(this.sublayerInfo.sublayers._items, function (sli) {
                        return sli.id;
                    });

                    //if (array.indexOf(this.sublayerInfo.subLayerIds, subLayerID) < 0) {
                    if (array.indexOf(subLayerIds, subLayerID) < 0) {
                        return false;
        ...
        _checkboxScaleRange: function () {
            var node = this.checkNode,
                ...
                scale = this.control.params.controller.view.scale,
