...
define([
    ...
    return {
        ...
        _legendRequest: function (layer, expandNode, callback, errback) {
            ...
            var url = layer.url + '/legend?f=pjson';
            var options = {
                responseType: 'json',
                method: 'post',
                useProxy: false,
        ...
        _createDynamicSublayerLegend: function (layer, expandNode, r) {
            ...
            array.forEach(r.data.layers, function (_layer) {
                ...
                //}, false))
                {
                    ...
                    if (layer.sublayers && layer.sublayers.length > 1) {
                        var node = registry.byId(layer.id + '-' + _layer.layerId + '-sublayer-control');
                        if (node) {
                            var sublayerExpandNode = node.expandNode;
        ...
        vectorLegend: function (layer, expandNode) {
            ...
            if (layer.renderer && layer.renderer.type) {
                if (layer.renderer.type == 'unique-value' && layer.renderer.uniqueValueInfos) {
                    this._createVectorLegend(layer.renderer.uniqueValueInfos, layer, expandNode);
                } else {
                    html.set(expandNode, 'No Legend');
                }
            }
        ...
        _createVectorLegend: function (infos, layer, expandNode) {
            ...
            array.forEach(infos, function (info) {
                ...
                if (info && info.symbol) {
                   var sym = info.symbol;
                   if (sym  && sym.source && sym.source.contentType) {
                      var contenttype = sym.source.contentType;
                      var imagedata = sym.source.imageData;
                      if (contenttype && imagedata) {
                         symbol.innerHTML = '<img style="display:inline;" src="data:' + contenttype + ';base64,' + imagedata + '"/>';
                      }
                   }
                }
